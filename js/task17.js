...
function renderChart() {
    ...
    for (var item in chartData) {
        ...
        text+='<div title="日期：'+item+',空气质量指数：'+chartData[item]+'" style="height:'+chartData[item]+'px;background-color:'+color+';"></div>';
    }
...
function initGraTimeForm() {
    ...
    for (var i=0;i<radio.length;i++) {
        addEvent(radio[i],'click',graTimeChange);
    }
}
    ...
    if (pageState.nowGraTime=='day') {
        ...
        for (var item in nowCityData) {
            if ((new Date(item)).getMonth()==month) {
                aqiSum+=nowCityData[item];
                daySum++;
            }else if ((new Date(item)).getMonth()!=month) {
                month++;
